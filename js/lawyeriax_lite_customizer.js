...
function lawyeriax_lite_refresh_general_control_values(){
	jQuery(".lawyeriax_general_control_repeater").each(function(){
		...
		th.find(".lawyeriax_general_control_repeater_container").each(function(){
			var icon_value = jQuery(this).find('.lawyeriax_icon_control').val();
...
jQuery(document).ready(function() {
	jQuery('.lawyeriax_icon_control').selectric({
		optionsItemBuilder: function (itemData, element, index) {
			return element.val().length ? '<i class="fa fa-2x ' + element.val() + '"></i>' + itemData.text : itemData.text;
		},
		onChange: function () {
